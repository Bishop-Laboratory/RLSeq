...
show_anno <- ifelse(tibble::is_tibble(params$anno_data), TRUE, FALSE)
show_corr <- ifelse(is.list(params$corr_data), TRUE, FALSE)
show_rlfs <- ifelse(is.list(params$rlfs_data), TRUE, FALSE)
show_readqc <- ifelse(is.list(params$read_qc_data), TRUE, FALSE)
...
rmap_samples <- readr::read_csv(rmap_samples)
...
peak_protocol <- paste0("Because the mode was <strong>", 
                        mode_now, "</strong> and there ", controlstr,
...
res <- suppressMessages(suppressWarnings(
  ChIPpeakAnno::makeVennDiagram(peak_ol, ignore.strand = FALSE,
                                margin = .05, fill = c('skyblue', 'firebrick'))))
```
...
peak_stats['Percentage Peaks Overlapping',] <- c(round(100*(
  peak_stats$MACS2[2]/sum(peak_stats$MACS2)), 1), 
  round(100*(peak_stats$EPIC2[2]/sum(peak_stats$EPIC2)), 1))
kableExtra::kable_styling(kableExtra::kable(peak_stats), 
