...
verd <- ifelse(
  pred$Verdict == "Case", 
  "<strong style='color: #602D64'>Case</strong>", 
  "<strong style='color: red'>Control</strong>"
)
```
<span style="font-size: 1.5em">Verdict for `r object@metadata$sampleName` is `r verd`.</span>
...
From these features, classification was performed to derive a *verdict* regarding whether the sample mapped R-loops or not. In short, "Case" indicates any sample for which all the following are true:
